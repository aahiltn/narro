...
function findClosestMatch(word: string, keywords: Set<string>): [string | null, number] {
...
async function gradeSentence(sentence: string, keywords: Set<string>): Promise<number> {
    ...
    const cleanedKeywords = new Set(
        Array.from(keywords).map(keyword => removePunctuation(keyword.toLowerCase()))
    );
    ...
    const closestMatches: { word: string; closestKeyword: string | null; similarity: number }[] = [];
    ...
    wordsInSentence.forEach(word => {
        if (cleanedKeywords.has(word)) {
            ...
            const [closestKeyword, similarity] = findClosestMatch(word, cleanedKeywords);
            closestMatches.push({ word, closestKeyword, similarity });

            const isMisspelled = matches.some((match: any) => sentence.slice(match.offset, match.offset + match.length) === word);
    ...
    closestMatches.forEach(({ word, closestKeyword, similarity }) => {
        console.log(`  - Word: ${word}, Closest Match: ${closestKeyword}, Similarity: ${similarity.toFixed(2)}`);
    });

    console.log(`Errors Identified by LanguageTool:`);
    matches.forEach((match: any) => {
        console.log(`  - Error: ${match.message}`);
        console.log(`    Word: "${sentence.slice(match.offset, match.offset + match.length)}"`);
        console.log(`    Rule: ${match.rule.description}`);
        console.log(`    Suggestion: ${match.replacements ? match.replacements.map((r: any) => r.value).join(', ') : "None"}`);
